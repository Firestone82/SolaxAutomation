...
public class WeatherQualityChecker {
    ...
    public void adjustModeBasedOnWeather() {
        ...
        if (now.getHour() == 7) {
            ...
            runCheck(now.withHour(9), now.withHour(12), properties.getThreshold().getCloudy(), 10, weatherCheck());
            return;
        }

        if (now.getHour() == 9) {
            logSeparator("Post-Morning weather forecast check");
            runCheck(now.withHour(10), now.withHour(14), properties.getThreshold().getCloudy(), 20, weatherCheck());
            return;
        }
